...
#include "pacecat_m_series_inter/msg/custom_point.hpp"
#include "pacecat_m_series_inter/srv/control.hpp"
...
typedef struct
{
  ...
  rclcpp::Publisher<pacecat_m_series_inter::msg::CustomMsg>::SharedPtr pub_custommsg;
...
void PointCloudCallback(uint32_t handle, const uint8_t dev_type, const LidarPacketData *data, void *client_data)
{
  ...
  if (data->data_type == LIDARPOINTCLOUD)
  {
    ...
    {
      pacecat_m_series_inter::msg::CustomMsg msg;
      ...
      {
        pacecat_m_series_inter::msg::CustomPoint point;
...
public:
      ...
      : Node("lidar_m_series")
  {
    ...
    READ_PARAM(int,"pfp_enable", pfp.pfp_enable, 1);
    READ_PARAM(int,"window_num", pfp.window_num, 10);
    READ_PARAM(double,"threshold", pfp.threshold, static_cast<double>(1.0));
    READ_PARAM(double,"effective", pfp.effective, static_cast<double>(2.0));
    ...
    devID = PaceCatLidarSDK::getInstance()->AddLidar(argdata, sfp, dfp, mr_2,pfp);
    ...
    // 创建服务端
    service = this->create_service<pacecat_m_series_inter::srv::Control>("scan_conntrol", std::bind(&LidarNode::control, this, _1, _2));
  }

private:
  rclcpp::Service<pacecat_m_series_inter::srv::Control>::SharedPtr service;
  // ArgData argdata;
  PubTopic pubtopic;
  int devID;
  void control(const pacecat_m_series_inter::srv::Control::Request::SharedPtr req, const pacecat_m_series_inter::srv::Control::Response::SharedPtr res)
  {
