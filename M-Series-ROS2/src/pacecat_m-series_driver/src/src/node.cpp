...
#include <thread>
#include <mutex>
#include <pacecat_m_series/CustomMsg.h>
#include <pacecat_m_series/CustomPoint.h>
#include <queue>
...
void PointCloudCallback(uint32_t handle, const uint8_t dev_type, const LidarPacketData *data, void *client_data)
{
  ...
  if (data->data_type == LIDARPOINTCLOUD)
  {
    ...
    {
      pacecat_m_series::CustomMsg msg;
      ...
      {
        pacecat_m_series::CustomPoint point;
...
int main(int argc, char **argv)
{
  ...
  nh.param("pfp_enable", pfp.pfp_enable, 1);
  nh.param("window_num", pfp.window_num, 10);
  nh.param("threshold", pfp.threshold, 1.0);
  nh.param("effective", pfp.effective, 2.0);
  ...
  if (pubtopic.output_custommsg)
    pubtopic.pub_custommsg = nh.advertise<pacecat_m_series::CustomMsg>(pubtopic.topic_custommsg, 10);
